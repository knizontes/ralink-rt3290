	...
	IN  PRTMP_ADAPTER   pAd)
{
	pAd->RalinkCounters.LastOneSecRxOkDataCnt = pAd->RalinkCounters.OneSecRxOkDataCnt;
		...
		NdisZeroMemory(&pAd->RalinkCounters,
						(void *)(uintptr_t)&pAd->RalinkCounters.OneSecEnd -
						(void *)(uintptr_t)&pAd->RalinkCounters.OneSecStart);
...
#ifdef RT3290
	if(pAd->AntennaDiversityInfo.TrainCounter > 0)
		pAd->AntennaDiversityInfo.TrainCounter--;
#endif
			...
			if (pAd->StaCfg.bHardwareRadio == TRUE)
			{
				if (pAd->StaCfg.bRadio != (pAd->StaCfg.bHwRadio && pAd->StaCfg.bSwRadio))
				{
					...
					if (pAd->StaCfg.bRadio == TRUE)
					{
						...
						{
							pAd->StaCfg.bDelayedPhyPowerIndication = TRUE;

							DBGPRINT(RT_DEBUG_TRACE,("%s: Postpone the PHY power indication to the RadioOnExec function.\n", __FUNCTION__));
						}
						else
						{
				...
				//
				// Indication will be made only when power state is changed
				// IndicateCurrentPhyPowerState(...) will check if the HW/SW radio was not change, then it will not indicate to OS
				//
				if (pAd->StaCfg.bDelayedPhyPowerIndication == FALSE)
				{
					//IndicateCurrentPhyPowerState(pAd, pPort, DOT11_PHY_ID_ANY);
				}
			}
		}
	}
...
#ifdef RT3290
	if (IS_RT3290(pAd) && (INFRA_ON(pAd) || ADHOC_ON(pAd)))
	{

		if (pAd->WlanBTCoexInfo.RateSelectionForceToUseRSSI == TRUE)
		{
			pAd->WlanBTCoexInfo.RateSelectionForceToUseRSSI = FALSE;
		}
	}
	
	if (IS_RT3290(pAd) && ((pAd->Mlme.OneSecPeriodicRound % 7) == 0) &&
		(INFRA_ON(pAd) || ADHOC_ON(pAd) || BT_ON(pAd)))
	{
		if (pAd->WlanBTCoexInfo.TxQualityFlag == 1)
		{
			pAd->WlanBTCoexInfo.TxQualityFlag = 2;
		}
		else if (pAd->WlanBTCoexInfo.TxQualityFlag == 2)
		{
			pAd->WlanBTCoexInfo.TxQualityFlag = 0;
		}		
	}
#endif /*RT3290*/
	return;
