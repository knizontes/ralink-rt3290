	...
	IN UCHAR FromWhichBSSID)
{
	...
	if ((skb =
		...
		skb_reserve(skb, 2);
		NdisMoveMemory((void *)(uintptr_t) skb->tail, pHeader802_3, HdrLen);
		skb_put(skb, HdrLen);
		NdisMoveMemory((void *)(uintptr_t)skb->tail, pData, DataSize);
	...
	IN ULONG DataSize)
{
	...
	if (pClonedPkt) {
		...
		pClonedPkt->tail = (uintptr_t)(pClonedPkt->data + pClonedPkt->len);
...
/*	ASSERT(pRxBlk->pRxPacket); */
	...
	pOSPkt->data = pData;
	pOSPkt->len = DataSize;
	pOSPkt->tail = (uintptr_t)(pOSPkt->data + pOSPkt->len);
